...
describe('invalid rlps', function () {
  const errCases = [
    ...
    },
    {
      ...
      msg: 'invalid RLP encoding', // not enough bytes for string length
    },
    {
      input: Buffer.from('efb83600', 'hex'),
      msg: 'invalid RLP encoding', // expected string length to be greater than 5
    },
    {
      input: Buffer.from(
      ...
      msg: 'invalid RLP encoding', // not enough bytes for string
    },
  ]
  errCases.forEach(({ input, msg }, index) => {
    it(`should not crash on an invalid rlp - ${index}`, function () {
      ...
      assert.ok(had_error, "decode() didn't throw an error");
    })
  })
})
